...
// ExchangeDiscordCodeForID provides a mock function with given fields: ctx, config, code
func (_m *TokenService) ExchangeDiscordCodeForID(ctx context.Context, config oauth2.Config, code string) (string, error) {
	ret := _m.Called(ctx, config, code)

	if len(ret) == 0 {
		panic("no return value specified for ExchangeDiscordCodeForID")
	}

	var r0 string
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, oauth2.Config, string) (string, error)); ok {
		return rf(ctx, config, code)
	}
	if rf, ok := ret.Get(0).(func(context.Context, oauth2.Config, string) string); ok {
...
// ExchangeYandexCodeForID provides a mock function with given fields: ctx, config, code
func (_m *TokenService) ExchangeYandexCodeForID(ctx context.Context, config oauth2.Config, code string) (string, error) {
	ret := _m.Called(ctx, config, code)

	if len(ret) == 0 {
		panic("no return value specified for ExchangeYandexCodeForID")
	}
